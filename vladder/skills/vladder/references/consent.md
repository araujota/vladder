...
- `canonical_training_data=opt_in`: at every eligible opportunity, contribute all supported,
  ...
  edges and topology; structured grammar actions; parent/child search lineage; completeness authority;
  direct and descendant utility; proof, differential, compile, assembly, cost, counter, benchmark, and
...
The model-training v3 disclosure is structural and must be described accurately. It removes
...
a distinctive normalized graph or search strategy may fingerprint an algorithm and records are linkable within one
...
The command privately pseudonymizes identity and maps flat prior evidence into bounded partial v3
searches. Use `training from-search-trace` when an authoritative engine can preserve full branch
lineage. Incomplete negative branches remain `KEEP_UNCERTAIN`. The command
